...
Then **audit the binary** after the first EAS archive (Xcode Organizer → Privacy Report, or Transporter). If Expo or a library shows UserDefaults / file timestamps, that is already declared in `app.json` privacyManifests and is **not** “collecting user data.” If the report shows Contact Info, Identifiers, Usage Data, or Diagnostics you did not expect, **stop and fix dependencies** before submit.
...
Wait for the Expo build to finish. Download the IPA / open in Xcode Organizer if you want to inspect.

**Confirm in the archive (Xcode → Organizer → the archive → Show in Finder → inspect, or Privacy Report):**

- Release configuration (no “development build” yellow banner)
- **No** Push Notifications entitlement (`aps-environment` absent)
- **No** `remote-notification` / audio background mode
- **No** Photos usage string
- Privacy manifest present (UserDefaults + file timestamp)
- `ITSAppUsesNonExemptEncryption` = false

Expo Go and `preview` / `development` profiles **cannot** go to the App Store.

- [ ] Production EAS build succeeded
- [ ] Archive audit: no push, no photos, no background audio
